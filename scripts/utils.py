...
def prepare_data(
    ...
    tr_val_stns, test_stns, *_ = TrainTestSplit(seed=seed).split_by_random(dyn['OBJECTID'].unique())
    tr_stns, val_stns, *_ = TrainTestSplit(seed=seed).split_by_random(tr_val_stns)
...
def get_fitted_model(model_type: str,
                     path: str,
                     metric: str):

    path = os.path.join(path, 'wrt_metrics', metric)
    ...
    wpath = os.path.join(path, 'weights')
    list_of_files = glob.glob(fr'{path}\weights\*.hdf5')
    latest_weights = max(list_of_files, key=os.path.getctime)
    model.update_weights(latest_weights)
