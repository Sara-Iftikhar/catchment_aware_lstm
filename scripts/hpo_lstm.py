...
PREFIX = f"hpo_lstm_{dateandtime_now()}"
ITER = 0
num_iterations = 200
lookback = 12
target = 'TP'
...
def objective_fn(
    ...
    layers = {
        "Input": {"batch_shape": (suggestions["batch_size"], lookback, num_inputs)},
        "LSTM": {"config": {"units": suggestions['units'],
...
tr_x, tr_y, val_x, val_y, test_x, test_y = prepare_data(
    target=target, lookback=lookback, batch_size=optimizer.best_paras()['batch_size'],
    treat_cat_as_ts=True
)
