...
PREFIX = f"hpo_lstm_multiply_{dateandtime_now()}"
ITER = 0
num_iterations = 200
lookback = 12
target = 'TP'
...
def objective_fn(
    ...
    transformer = Transformations('log')
    ...
    layers = {
        ...
        "LSTM": {"config": {"units": suggestions["units"]}, "inputs": "Input_dyn"},
        "Dense_cat": {"config": {"units": suggestions["units"]}, "inputs": "Input_cat"},
